...
if __name__ == "__main__":
    ...
    if not os.path.exists(full_path):
        ...
        if action == 'org':
            ...
            Helper.backup_directory_structure(full_path)
            ...
            Helper.organize_files_by_extension(full_path, files, exceptions)
        elif action == 'undo':
            # Undo the last organization
            Helper.undo_last_organization(full_path)
        elif action == 'backup':
            # Create a backup of the directory structure
            Helper.backup_directory_structure(full_path)
        elif action == 'list':
            # List available backups
            Helper.list_backups(full_path)
        elif action == 'restore':
            # List available backups
            Helper.list_backups(full_path)
            ...
            try:
                ...
                if 0 <= choice_index < len(timestamps):
                    Helper.restore_directory_structure(full_path, timestamps[choice_index])
                else:
            ...
            try:
                ...
                timestamps = list(data['moves'].keys())
                if 0 <= choice_index < len(timestamps):
                    Helper.delete_backup(full_path, timestamps[choice_index])
                else:
                    print("Invalid choice.")
            except ValueError:
                print("Invalid input. Please enter a number.")
